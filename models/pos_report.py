...
class PosReportGenerator(models.Model):
    ...
    @api.model
    def pos_report(self, option,shop_id=None):
        ...
        shop_ids = self.env['pos.config'].search([])
        # shop_datas = [{'s_key':'ALL','s_value':'ALL'}]
        shop_datas = []
        for shop_id in shop_ids:
            shop_datas.append({'id':shop_id.id,'s_key':str(shop_id.name),'s_value':shop_id.name})
        return {
            'name': "PoS Orders",
    ...
    def _get_report_sub_lines(self, data, report, date_from, date_to):
        ...
        if shop_id=='ALL' or not shop_id:
            shop_ids = self.env['pos.config'].search([]).ids
            if len(shop_ids) > 1:
                shop_id = tuple(self.env['pos.config'].search([]).ids)
            elif len(shop_ids)== 1:
                shop_id = '('+str(self.env['pos.config'].search([]).id)+')'
        else:
            shop_id = '('+str(self.env['pos.config'].search([('name','=',shop_id)]).id)+')'
        if data.get('report_type') == 'report_by_order':
            ...
            query = '''
                    ...
                    where pos_session.config_id in %s
                             '''% str(shop_id)
            term = 'Where '
        ...
        if data.get('report_type') == 'report_by_order_detail':
            query = '''
            ...
            term = 'Where '
            if data.get('date_from'):
                query += "and pos_order.date_order >= '%s' " % data.get('date_from')
                term = 'AND '
            if data.get('date_to'):
                query += " and pos_order.date_order <= '%s' " % data.get('date_to')
