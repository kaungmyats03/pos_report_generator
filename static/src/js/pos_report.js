odoo.define('pos_report_generator.pos_report', function(require) {
	...
	var PosReport = AbstractAction.extend({
		...
		init: function(parent, action) {
			...
			this.shop_datas = action.shop_datas;
		...
		load_data: function(initial_render = true) {
			...
			if ($(".report_type").length) {
				shop_id = $(".report_type")[3].value
			}
			self._rpc({
				model: 'pos.report',
				method: 'pos_report',
				args: [
					[this.wizard_id],shop_id
				],
			}).then(function(datas) {
				if (initial_render) {
					self.$('.filter_view_pr').html(QWeb.render('posFilterView', {
						filter_data: datas['filters'],
						shop_datas: datas['shop_datas'],
...
//            event.preventDefault();
			...
			if ($(".report_type").length) {
				...
				console.log("####################");
				var shop_res = document.getElementById("shop_res")
				console.log($(".report_type"));
				console.log("aaaaaaaaaaaa");
				// filter_data_selected.shop_id = $(".report_type")[3].value
				shop_res.value = $(".report_type")[3].value
				shop_res.innerHTML = shop_res.value;
